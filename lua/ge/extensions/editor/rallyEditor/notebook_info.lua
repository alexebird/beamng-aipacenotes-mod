...
function C:drawCodriverForm(codriver)
  ...
  -- editEnded = im.BoolPtr(false)
  -- editor.uiInputText("Voice", codriverVoiceText, nil, nil, nil, nil, editEnded)
  -- if editEnded[0] then
  --   editor.history:commitAction("Change Voice of Codriver",
  --     {self = self, index = self.codriver_index, old = codriver.voice, new = ffi.string(codriverVoiceText), field = 'voice'},
  --     setCodriverFieldUndo, setCodriverFieldRedo)
  -- end
...
function C:loadVoiceFile(voiceFname)
  ...
  if not voices then
    log('E', logTag, 'unable to load voices file from ' .. tostring(voiceFname))
    return {}
  end
