...
local editModeName = "Edit Rallies"
local im = ui_imgui
local ffi = require('ffi')
local roadRiverGui = extensions.editor_roadRiverGui
local currentMode = 'Pathnodes'
local previousFilepath = "/gameplay/rally/"
local previousFilename = "NewRally.rally.json"
local windows = {}
local currentWindow = {}
local testingWindow
local currentPath = require('/lua/ge/extensions/gameplay/rally/path')("New Rally")
currentPath._fnWithoutExt = 'NewRally'
...
local function raceTest(dtReal, dtSim, dtRaw)
  ...
  im.Begin("Rally Test", raceTestWindowOpen)
    ...
    if im.BeginMenuBar() then
      if im.BeginMenu("File") then
        ...
        if im.MenuItem1("Load...") then
          editor_fileDialog.openFile(function(data) loadRace(data.filepath) end, {{"Rally files",".rally.json"}}, false, previousFilepath)
        end
        ...
        if im.MenuItem1("Save as...") then
          extensions.editor_fileDialog.saveFile(function(data) saveRace(currentPath, data.filepath) end,
                                        {{"Rally files",".rally.json"}}, false, previousFilepath)
        end
        if im.MenuItem1("Clear") then
          local path = require('/lua/ge/extensions/gameplay/rally/path')("New Rally")
          editor.history:commitAction("Set path to new path.",
            {path = path, fp = "/gameplay/rally/", fn = "new.path.json"},
            setRaceUndo, setRaceRedo)
        end
        ...
        if im.BeginMenu("All Rallies...") then
          if im.SmallButton("Refresh (!)") then
            ...
            for _, f in ipairs(FS:findFiles("/", '*.rally.json', -1, true,true)) do
...
local function onEditorInitialized()
  ...
  table.insert(windows, require('/lua/ge/extensions/editor/rallyEditor/pathnodes')(M))
...
local function onDeserialized(data)
  if data then
    ...
    previousFilename = data.previousFilename  or "NewRally.rally.json"
    previousFilepath = data.previousFilepath or "/gameplay/rally/"
    currentPath._dir = previousFilepath
    local dir, filename, ext = path.splitWithoutExt(previousFilename, true)
    currentPath._fnWithoutExt = filename
