...
--     return
--   end
--   local path_path = '/lua/ge/extensions/gameplay/race/path'

--   local function stringEndsWith(str, ending)
--     return ending == "" or string.sub(str, -string.len(ending)) == ending
--   end

--   if stringEndsWith(file, 'rally.json') then
--     path_path = '/lua/ge/extensions/gameplay/rally/path'
--   end

--   log('D', 'wtf', path_path)

--   local path = require(path_path)("New Race")
...
function C:getTrackedData()
  ...
  for id, data in pairs(self.states) do
    if data.complete then
      ...
      --     data.offPos = vec3(0,0,0)
      --     data.offPos:set(data.frontPos)
      ...
      --     local distCut = self:getDistOnNav(data.offPos, data.frontPos, data.inNextAiPathIdx, data.inPrevAiPathIdx, data.aiPathIdx + 1, data.aiPathIdx)
      --   end
      -- end
