...
function C:resetAudioQueue()
  ...
  local resp = extensions.gameplay_aipacenotes_client.remote_audio_reset()
  if not resp.ok then
    log('E', logTag, "_enqueueFileRemote: not resp.ok")
  end
end
...
function C:enqueuePacenote(pacenote, addToFront)
  ...
  if pacenoteFgData then
    ...
    return self:_enqueueFileRemote(pacenoteFgData.audioFname, addToFront)
  else
...
function C:_enqueueFileRemote(fname, addToFront)
  addToFront = addToFront or false
  -- local audioObj = re_util.buildAudioObjPacenote(fname)
  if re_util.fileExists(fname) then
    log('D', logTag, "_enqueueFile: exists=yes front="..tostring(addToFront) .." fname=" .. fname)
    if addToFront then
      ...
      -- self.queue:push_right(audioObj)
      local resp = extensions.gameplay_aipacenotes_client.remote_audio_play_file(fname)
      if not resp.ok then
        log('E', logTag, "_enqueueFileRemote: not resp.ok")
      end
    end
